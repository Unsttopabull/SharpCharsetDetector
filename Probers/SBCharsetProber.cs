...
namespace Frost.SharpCharsetDetector.Probers {
    ...
    public class SingleByteCharSetProber : CharsetProber {
            ...
            for (int i = offset; i < max; i++) {
                ...
                if (order < SAMPLE_SIZE) {
                    ...
                    if (_lastOrder < SAMPLE_SIZE) {
                        _totalSeqs++;
                        if (!_reversed) {
                            ++(_seqCounters[Model.GetPrecedence(_lastOrder * SAMPLE_SIZE + order)]);
                        }
                        else // reverse the order of the letters in the lookup
                        {
                            ++(_seqCounters[Model.GetPrecedence(order * SAMPLE_SIZE + _lastOrder)]);
                        }
                    }
                }
                _lastOrder = order;
        ...
        public override void DumpStatus() {
            Console.WriteLine("  SBCS: {0} [{1}]", GetConfidence(), CharsetName);
        }
        ...
        public override float GetConfidence() {
            ...
            */
            ...
            if (_totalSeqs > 0) {
                float r = 1.0f * _seqCounters[POSITIVE_CAT] / _totalSeqs / Model.TypicalPositiveRatio;
        ...
        public override int WindowsCodePage {
            get {
                return (_nameProber == null)
                    ? Model.WindowsCodePage
                    : _nameProber.WindowsCodePage;
            }
        }
    }
