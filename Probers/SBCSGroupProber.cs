...
using System;
using Frost.SharpCharsetDetector.Models.SequenceModels.Bulgarian;
using Frost.SharpCharsetDetector.Models.SequenceModels.Cyrillic;
using Frost.SharpCharsetDetector.Models.SequenceModels.Greek;
using Frost.SharpCharsetDetector.Models.SequenceModels.Hebrew;
...
namespace Frost.SharpCharsetDetector.Probers {
    ...
    public class SBCSGroupProber : CharsetProber {
        ...
        public SBCSGroupProber() {
            ...
            // Logical  
            _probers[11] = new SingleByteCharSetProber(new Win1255Model(), false, hebprober);

            // Visual
            _probers[12] = new SingleByteCharSetProber(new Win1255Model(), true, hebprober);
            ...
            for (int i = 0; i < PROBERS_NUM; i++) {
                if (!_isActive[i]) {
                    Console.WriteLine(" inactive: [{0}] (i.e. confidence is too low).",
                        _probers[i].CharsetName);
                }
                else {
                    _probers[i].DumpStatus();
                }
            }
            Console.WriteLine(" SBCS Group found best match [{0}] confidence {1}.", _probers[_bestGuess].CharsetName, cf);
        }
