        ...
        */
        ...
        public override ProbingState HandleData(byte[] buf, int offset, int len) {
            ...
            for (int i = 0; i < newbuf.Length; i++) {
                byte charClass = Latin1CharToClass[newbuf[i]];
                byte freq = Latin1ClassModel[_lastCharClass * CLASS_NUM + charClass];
                if (freq == 0) {
        ...
        public override void DumpStatus() {
            Console.WriteLine(" Latin1Prober: {0} [{1}]", GetConfidence(), CharsetName);
        }
    }
